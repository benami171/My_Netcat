...
void RUN(char *args_as_string) {
    ...
    while (token != NULL) {
        ...
        if (temp == NULL) {                                       // check if realloc failed
            free(args);                                           // free the original memory block
            fprintf(stderr, "Memory allocation error\n");         // print an error message
            exit(EXIT_FAILURE);                                   // exit the program with a failure status
        }
        args = temp;                                              // assign the reallocated memory to args
...
int main(int argc, char *argv[]) {
    ...
    if (ovalue != NULL)  // changin the output to the one who we're addressing
    {
        ...
        char *ip_server = strtok(ovalue, ",");  // getting the ip like this TCPC127.0.0.1,8080
        if (ip_server == NULL) {
        ...
        sock_output = sock;  // changing the output to form the socket to the client
    }
